...
//#endif
	...
	u16    SendStaticPicture(BOOL32 bSend, s8* pszFileName);
 	u16    GetFirstWaveInAudioDevice(s8 * pszDeviceName); // 返回值ERROR_NO_AUDIO_DEV表示无声卡
	u16    GetNextWaveInAudioDevice(s32 nDeviceID, s8 * pszDeviceName);  // 返回值ERROR_NO_AUDIO_DEV表示无声卡
...
protected:
	...
	char   m_pszAudioDevList[MAX_AUDIO_DEV_NUMBER][MAX_AUDIO_DEV_NAME_LEN];
    TEncoderRef      *m_ptRef;
...
//	u16    SetWaveOutAudioDevice(CString strContainText);
 	u16    GetFirstWaveOutAudioDevice(s8 * pszDeviceName); // 返回值ERROR_NO_AUDIO_DEV表示无声卡
	u16    GetNextWaveOutAudioDevice(s32 nDeviceID, s8 * pszDeviceName);  // 返回值ERROR_NO_AUDIO_DEV表示无声卡
...
#ifdef SOFT_CODEC
	//设置空闲时播放静态图片
	BOOL32 SetStaticPicturePath(s8 *pszFilePath);
#endif
