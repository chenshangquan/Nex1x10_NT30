...
/*For KDV9000A MSU*/
    u8 abyRS232_1_Name[TTY_NAME_MAX_LEN];/*设备RS232串口对应的设备名*/
...
/* KDV9000A SMM 指示灯结构，成员变量值如宏定义：指示灯状态 */
...
/*
 *  主备切换控制
 *  1) 竞争主控权: 先将本板设置为host，然后再检测本板的主备状态，如果检测为host，
 *     则表示成功获取到主控权，否则作为备板使用。
 *  2) 放弃主控权: 直接将本板设置为slave。如果对板原先为slave，而且正在竞争host，
 *     则对板将获取到主控权，即对板检测为host；否则对板原先为host的，将继续保持。
 *
 *  注意: 初始情况下两块板子都为备板，由业务这边开启主备竞争并进行状态控制。
 */
...
/*
 *  机框上面板子在位信息查询，每一个比特代表一块板子:
 *
 *  [0]~[5],[8]~[13] --- 1~6,9~14插槽位，LPU 媒体板;
 *  [6]~[7] --- 7~8插槽位，MSU媒体交换板;
 *  [14]~[17] --- 机框AC电源，PEM0PRES ~ PEM3PRES;
 *  [18] --- 防尘网;
 *  [19] --- 另外一块SMM板的在位状态。
 *  [20]~[25] --- 风扇模块，FAN0_PRES ~ FAN5_PRES;
 */
...
/*
 * 主备板健康状态查询与控制
 * 1) 设置本板的健康状态: 可以设置为健康或者不健康，不可以由对板设置，也不可以
 *    设置对板的健康状态。
 * 2) 查询本板和对板的健康状态: 可以查询本板或者对板的健康状态。
 */
...
#if 1
#define FUNCDISABLE        1       /*状态禁止  */
#define FUNCENABLE       0       /*状态使能  */

#define DISTORT_1BTYE   (unsigned char)0        /* 畸变功能地址 */
#define END_1BTYE           DISTORT_1BTYE+1 /*截止地址 */
...
/*================================
函数名：BrdE2promGetUserdata
功能：获取用户自定义数据
算法实现：（可选项）
引用全局变量：
输入参数说明：cmd:功能号，buf:返回的数据
返回值说明： 成功返回0，否则返回-1
==================================*/
...
/* =====================================
函数名：BrdEEpromGetFlagBit
功能：获取标志位flag位pos状态
参数说明:flag为0表示为标志位0，flag为1表示标志位为1
算法实现：（可选项）
引用全局变量：
返回值说明：
				获取失败返回-1
...
/* =====================================
函数名：BrdEEpromSetFlagBit
功能：设置标志位flag位pos状态值value
参数说明:flag为0表示为标志位0，flag为1表示标志位为1
算法实现：（可选项）
引用全局变量：
返回值说明：
...
/*====================================================================
 函数名: BcsrGetMSUMainMode
 功能  : 获取msu板子的主备状态
 输入参数: isMain -- 用于获取板子主备的指针
           #define BRD_MSU_STANDBY_MODE		0	isMain为0，板子为备板
           #define BRD_MSU_MAIN_MODE		1	isMain为1，板子为主板
           #define BRD_MSU_MAIN_MODE_ERR	0xFF	isMain为0xFF，主备状态错误                           
 返回值说明: 0为正常，-1为异常
 ----------------------------------------------------------------------
 修改记录    ：日  期      版本        修改人        修改内容
               05/27/14    1.0          张专          创建
 ====================================================================*/
/*KDV9000A MSU主备状态*/
#define BRD_MSU_STANDBY_MODE    0    /*MSU 备板状态*/ 
#define BRD_MSU_MAIN_MODE       1    /*MSU 主板状态*/
#define BRD_MSU_MAIN_MODE_ERR   0xFF /*MSU 主备状态错误*/
