...
功能：注册IP/MAC地址冲突时回调函数,当设备的IP/MAC地址和外部设备冲突时，会调用该函数通知业务程序
算法实现：（可选项）
引用全局变量：
...
/*===============================
函数名：BrdTelnetOnOffSwitch
功能:Telnet开关函数
算法实现：（可选项）
引用全局变量：
输入参数说明：telSwh = 0 :重启后关闭telnet  
                     = 1 :重启后开启telnet
返回值说明:      0 : ok
                -1 : error
================================*/
