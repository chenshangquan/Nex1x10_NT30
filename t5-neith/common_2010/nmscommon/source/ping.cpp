...
#define TRANSLATE(text, lan) _tcscmp(lan, _T("chinese")) == 0 ? text##CHN : text##ENG

    #define TIMES_ERR_CHN           _T("测试次数必须介于1-255之间！")
    #define TIMES_ERR_ENG           _T("Test times should between t1 and 255!")
#define TIMES_ERR(lan)          TRANSLATE(TIMES_ERR_, lan)
    #define PACKSIZE_ERR_CHN        _T("测试包长必须介于0-10K字节之间！")
    #define PACKSIZE_ERR_ENG        _T("Package size should between 0 and 10K bytes!")
#define PACKSIZE_ERR(lan)       TRANSLATE(PACKSIZE_ERR_, lan)
    #define RESTART_CHN             _T("内部错误，请重启服务！")
    #define RESTART_ENG             _T("Internal error! Please restart the service!")
#define RESTART(lan)            TRANSLATE(RESTART_, lan)
    #define NO_ERR_CHN              _T("操作成功")
    #define NO_ERR_ENG              _T("Operation Success")
...
BOOL32 CPing::Start(void)
{
    ...
    for ( nIndex = 0; nIndex < m_nTimes; nIndex ++ )
    {
		...
		tTestEnd.m_nJitter=(s32)sqrt((float)nJitterSum/tTestEnd.m_nSucCount);
        bStop = ! m_pfnResult( m_pcbData, tPingRst );
        if ( bStop )
        {
            nIndex ++;
...
2006/06/05  4.0     王昊    创建
=============================================================================*/
LPCTSTR CPing::GetErrStr(LPCTSTR lptrLan)
{
    switch ( m_byErrCode )
    {
    case emTimesErr:
    ...
    case emNetErr:
        {
            ...
            DWORD dwLangID = 0;
            if ( _tcscmp(lptrLan, _T("chinese")) == 0 )
            {
            ...
            if ( FormatMessage( FORMAT_MESSAGE_ALLOCATE_BUFFER | 
                                ...
                                NULL ) > 0 )
            {
                _tcsncpy( m_aszErrStr, lpMsgBuf, _countof(m_aszErrStr) - 1 );
                m_aszErrStr[ _countof(m_aszErrStr) - 1 ] = _T('\0');
                LocalFree( lpMsgBuf );
                return m_aszErrStr;
            }
            else
