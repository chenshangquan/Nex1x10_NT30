                                    ...
                                    char *lpReturnedString, unsigned int nSize)
{
    ...
    for ( ;itr != m_vctData.end(); itr++ )
    {
        ...
        {
            if ( nSize > ptData->m_strVal.size() && !ptData->m_strVal.empty() )
            {
				strcpy( lpReturnedString, ptData->m_strVal.c_str() );
...
int CParseCfgfile::GetProfileInt(const char *lpAppName, const char *lpKeyName, int nDefault)
{
    ...
    for ( ;itr != m_vctData.end(); itr++ )
    {
        PTCfgFileData ptData = &(*itr);
        if ( ptData->m_strSec == lpAppName && ptData->m_strKey == lpKeyName 
			&& false == ptData->m_strVal.empty() )
        {
            int nRet = atoi( ptData->m_strVal.c_str() );
            return nRet;
        }
    }
