...
const char* CPrintCtrl::GetTime()
{
	...
	_snprintf( achTime, sizeof(achTime), "%4d/%.2d/%.2d %.2d:%.2d:%.2d\n", _tmp->tm_year, _tmp->tm_mon, _tmp->tm_mday,
...
BOOL32 CPrintCtrl::IsPrintMsg( const u16 wMsgID )
{ 
	if ( m_bPrintAllMsg )
	{
		return TRUE;
	...
	if ( m_dwPrintMsgID == wMsgID)
	{
		return TRUE;
	}

	if ( m_dwPrintRangeMin <= wMsgID 
		&& wMsgID <= m_dwPrintRangeMax) 
	{
		return TRUE;
	} 

   map<u16, u8>::iterator itr = m_mapEventLevel.find(wMsgID);
