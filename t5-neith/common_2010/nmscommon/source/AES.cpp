...
AES::~AES()
{
    if( State) delete [] (State - (this->isdecrypt?3:0));
    if( Word ) delete [] (Word);
}
...
int AES::setMode(
        ...
        bool     isdecrypt)
{
    if( !((keySize == len16 || keySize == len24 || keySize == len32)&& 
          (inSize  == len16 || inSize  == len24 || inSize  == len32)))
    {
        return -1;
    }
    ...
    State = new unsigned long[Nb+3];
//	printf("State加密或解密前地址:%x\n",State);
    if(isdecrypt) State += 3;
